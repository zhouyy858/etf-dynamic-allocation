...
def main():
    ...
    lines.append("")
    lines.append("> 生成: ETF动态配置skill v30（周五前日信号决策+1笔当日成交+溢价T-2门控+相对溢价倾斜+溢价门控削减增强+相关性风控+速度刹车(恢复期12日)+逆回购2.2%+国债ETF 511010七成半+前提驱动+v30定稿:底仓2+10/创业板30/纳指63）｜ 无未来函数口径｜ 仅供参考，非投资建议")
    text = "\n".join(lines)
    print(text)
    if out_path:
