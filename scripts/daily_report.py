...
CFG_FILE = os.path.join(HERE, "..", "references", "final_cfg_v23.json")
...
def main():
    ...
    if is_rebal_day:
        action = f"今日是周{WEEKDAY_CN[rebal_wd][1]}调仓日：按 v23 规则用前一日(T-1)收盘信号决策，目标缺口 1 笔当日收盘成交；QDII 溢价用 T-2 口径（>3% 注意、>5% 买入暂缓），下单前查 IOPV"
    elif gap > 0.02:
    ...
    lines.append("")
    lines.append("> 生成: ETF动态配置skill v23（周五前日信号决策+1笔当日成交+溢价T-2门控+相关性风控+速度刹车+逆回购2.2%+国债ETF5成+v23定向优化定稿）｜ 无未来函数口径｜ 仅供参考，非投资建议")
    text = "\n".join(lines)
    print(text)
    if out_path:
