# -*- coding: utf-8 -*-
"""v24 激进档(f=1.1)全量验证: proxy/real/WFO/压力 vs v24 标准档
f=全局权益激进系数: state_map growth/defense 等比缩放(上限98/40)
"""
...
def main():
    ...
    agg = scale_state(base, 1.1)
    agg["version"] = "v24-aggressive"
    VAR = {"v24标准": base, "v24激进f1.1": agg}
    print("===== 全历史 + real + WFO =====")
    ...
    for tag, c in VAR.items():
        ...
        print(line)
    json.dump(rows, open(f"{OUT}/exp_v24_aggressive.json", "w"), ensure_ascii=False, indent=1)
    print(f"\n[ok] {OUT}/exp_v24_aggressive.json")
